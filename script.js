...
document.querySelectorAll('a,button,.pj,.pj-full,.sc2,.ftab').forEach(el=>{el.addEventListener('mouseenter',()=>cr.classList.add('h'));el.addEventListener('mouseleave',()=>cr.classList.remove('h'))});
window.addEventListener('scroll',()=>document.getElementById('nav').classList.toggle('sc',scrollY>60));
const hbg=document.getElementById('hbg');if(hbg)setTimeout(()=>hbg.classList.add('go'),100);
...
function olb(el){const lb=document.getElementById('lb');if(!lb)return;document.getElementById('lbi').src=el.querySelector('img').src;document.getElementById('lbc').textContent=el.querySelector('.pname').textContent+' · '+el.querySelector('.ploc').textContent;lb.classList.add('op');document.body.style.overflow='hidden'}
function clb(){const lb=document.getElementById('lb');if(lb)lb.classList.remove('op');document.body.style.overflow=''}
const lb=document.getElementById('lb');if(lb)lb.addEventListener('click',function(e){if(e.target===this)clb()});
